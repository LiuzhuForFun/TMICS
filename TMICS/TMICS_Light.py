...
class NNalignedModule2(torch.nn.Module):
      def __init__(self, c, cin=15):
          super(NNalignedModule2, self).__init__()
          # Resblock groups
          self.preconv = torch.nn.Conv2d(in_channels=cin, out_channels=c, kernel_size=3, stride=1, padding=1)
          # the blocks
...
class TOFlow(torch.nn.Module):
    def __init__(self, h, w, args, task, cuda_flag, genotype):
        ...
        C = 96
        self.feature = NNalignedModule2(C)
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(6, C, 3, padding=1, bias=True)
        )
    ...
    def forward(self, frames, iters=12, test_mode=False, opticalflow = True):
        ...
        if opticalflow:
            ...
            x = warpframes[:, 2, :, :, :]
            frames = torch.cat((warpframes[:, 0, :, :, :], warpframes[:, 1, :, :, :], warpframes[:, 2, :, :, :], warpframes[:, 3, :, :, :], warpframes[:, 4, :, :, :]), dim=1)

            res = self.feature(frames) # the Features
            res_1 = self.stem_out(res)
            # s1 = self.decom(res)
            # s1 = self.decom2(s1)
            # Residule
            #
            # print( np.shape(self.conv[1, :, :, :, :] / 10))
            rain_streak = F.conv2d(res_1, self.conv[1, :, :, :, :] / 10, stride=1,padding=4)  # self.conv[1,:,:,:,:]：rain kernel is inter-stage sharing
            #residule and orignal
            x_rob = x - rain_streak
            x_concat = torch.cat([x,x_rob],dim=1)
            s1 = self.stem(x_concat)
            s1 = self.cell_1(s1)
            s1 = self.cell_2(s1)
            s1 = self.cell_3(s1)
            s1 = self.cell_4(s1)
...
class TMICS(torch.nn.Module):
    ...
    def __init__(self, h, w, args, task, cuda_flag, genotype, model_path):
    ...
    def forward(self, frames, iters=12, test_mode=False, opticalflow = True):
        ...
        # setting the weights
        alpha1 = alpha1*1e-5
        return (1-alpha1)*img2 + alpha1* img1